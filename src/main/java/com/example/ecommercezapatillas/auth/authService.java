...
public class authService {
    ...
    public AuthResponse register(RegisterRequest request) {
     ...
     @Transactional
    public List<Direccion> getUserAddresses(Long userId) throws UsernameNotFoundException {
    ...
    public UserDTO updateUser(Long id, UpdateUserRequest request) throws Exception {
        ...
        // La propiedad se llama 'active' en el DTO, y para booleanos Lombok genera
        // 'isActive()'
        // O si quieres que acepte nulo para no actualizar si no se envía:
        // if (request.getActive() != null) {
        // existingUser.setActive(request.getActive());
        // }
        // Si 'active' es un primitivo 'boolean' en el DTO, siempre tendrá un valor
        // (true/false)
        ...
        if (request.getProfileImage() != null && !request.getProfileImage().trim().isEmpty()) {
            if (!request.getProfileImage().equals(existingUser.getProfileImage())) {
                if (existingUser.getProfileImagePublicId() != null
                        && !existingUser.getProfileImagePublicId().isEmpty()) {
                    try {
                        cloudinaryService.deleteFile(existingUser.getProfileImagePublicId());
                    } catch (Exception e) {
                        System.err.println("Error al eliminar la imagen antigua de Cloudinary (ID: "
                                + existingUser.getProfileImagePublicId() + "): " + e.getMessage());
                    }
                }
            ...
            if (existingUser.getProfileImagePublicId() != null && !existingUser.getProfileImagePublicId().isEmpty()) {
                try {
                    ...
                    System.err.println("Error al eliminar la imagen de Cloudinary (quitada) (ID: "
                            + existingUser.getProfileImagePublicId() + "): " + e.getMessage());
                }
            }
